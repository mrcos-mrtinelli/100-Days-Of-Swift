...
class ViewController: UICollectionViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    override func viewDidLoad() {
        ...
        if let savedJSONData = userDefaults.object(forKey: "people") as? Data {
            let jsonDecoder = JSONDecoder()
            
            do {
                people = try jsonDecoder.decode([Person].self, from: savedJSONData)
            } catch  {
                print("error")
            }
        }
    ...
    func saveAndReload() {
        ...
        if let saveData = try? jsonEncoder.encode(people) {
            ...
            userDefaults.set(saveData, forKey: "people")
