...
class ViewController: UITableViewController {
    ...
    @objc func getUserInput() {
        ...
        let searchAction = UIAlertAction(title: "Search", style: .default) {
            ...
            if let userInput = ac?.textFields?[0].text {
                self?.performSelector(inBackground: #selector(self?.filterRestuls), with: userInput.lowercased())
            }
        }
    ...
    @objc func filterRestuls(_ userInput: String) {
        ...
        DispatchQueue.main.async {
            [weak self] in
            guard let numberOfPetitions = self?.filteredPetitionList.count else { return }
            self?.title = "Viewing \(numberOfPetitions) petitions"
            self?.reloadTitle()
