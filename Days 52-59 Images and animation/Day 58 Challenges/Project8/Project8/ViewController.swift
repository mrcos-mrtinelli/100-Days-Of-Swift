...
class ViewController: UIViewController {
    ...
    override func loadView() {
        ...
        for row in 0..<4 {
            for column in 0..<5 {
                // set button to custom type - this stops the title from flashing when tapped
                let letterButton = UIButton(type: .custom)
                letterButton.addTarget(self, action: #selector(letterTapped), for: .touchUpInside)
                letterButton.titleLabel?.font = UIFont.systemFont(ofSize: 36)
                letterButton.setTitle("WWW", for: .normal)
                
                // set custom button's colors
    ...
    @objc func letterTapped(_ sender: UIButton) {
        ...
        // fade buttons out slowing after being tapped
...
//        sender.isHidden = true
    ...
    func loadLevel() {
        ...
        solutionsLabel.text = solutionString.trimmingCharacters(in: .whitespacesAndNewlines)
